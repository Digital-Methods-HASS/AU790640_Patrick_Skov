...
l_dk <- leaflet() %>% 
  setView(lng = 10.85089, lat = 55.2339084, zoom = 7) # Zoom justeret til at vise DK

# Tilføj Esri baggrunde fra dit tidligere 'esri' objekt
for (provider in esri) {
  l_dk <- l_dk %>% addProviderTiles(provider, group = provider)
}

# Saml det hele i DANmap
DANmap <- l_dk %>%
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  addMiniMap(tiles = esri[[1]], toggleDisplay = TRUE,
...
#Kør koden for at se dit DANmap
DANmap
...
# The unclustered map is for seeing the exact location
# But can be a hassle to navigate if there is large amounts of data

# The clustered map is good for seeing the general distribution of points
# But you cannot see the exact location of each point.
...
DANmap_final <- DANmap %>% 
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             clusterOptions = markerClusterOptions(), 
             popup = paste("<b>Lokation:</b>", places$Description, "<br>",
                           "<b>Type:</b>", places$Type, "<br>",
                           "<b>Klassifikation:</b>", places$Placename, "<br>",
                           "<b>Noter:</b>", places$Notes))
